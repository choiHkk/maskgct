...
from data import DataCollatorWithPadding, DynamicBatchSampler, prepare_datasets
...
def main():
    ...
    if pretrained_weight_repo_id is not None and last_checkpoint is None:
        model = MaskGCT_T2S.from_pretrained(pretrained_weight_repo_id)
    else:
        model = MaskGCT_T2S(
    ...
    def generate_step(batch, accelerator):
        ...
        inputs = {
            "target_length": batch["speech_input_ids"].shape[-1],
            "prompt_input_ids": batch["prompt_input_ids"],
            "speech_input_ids": batch["speech_input_ids"],
        }
