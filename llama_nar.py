...
from modeling_llama import (BaseModelOutputWithPast, LlamaConfig,
...
class DiffLlamaPrefix2(LlamaModel):
    def __init__(
        self,
        hidden_size=1024,
        num_heads=16,
    ...
    def forward(
        ...
        x = x + self.cond_mlp(cond)  # (B, T, C)

        phone_embedding = self.cond_mlp2(phone_embedding)  # (B, T, C)
        phone_length = phone_embedding.shape[1]
        inputs_embeds = torch.cat([phone_embedding, x], dim=1)
        attention_mask = torch.cat([phone_mask, x_mask], dim=1)
