...
from llama_nar import DiffLlamaPrefix, LlamaConfig
...
class MaskGCT_T2S(nn.Module, PyTorchModelHubMixin):
    def __init__(
        self,
        vocab_size,
        hidden_size=1024,
        ...
        self.to_logit = nn.Linear(self.hidden_size, self.codebook_size)

        self.cond_emb = nn.Embedding(codebook_size + 1, self.hidden_size)
    ...
    def init_phone_emb(self, vocab_size):
        old_emb = self.phone_emb.weight.data
        mean = old_emb.mean().item()
        std = old_emb.std().item()
        self.phone_emb = nn.Embedding(vocab_size, self.hidden_size)
        nn.init.normal_(self.phone_emb.weight, mean=mean, std=std)
    ...
    def loss_t(self, x0, x_mask, t, phone_embedding=None, phone_mask=None):
        ...
        return self.cross_entropy(logits=logits, targets=x0, mask=final_mask)
    ...
    def generate(
        ...
        **kwargs,
        ...
        generated_codes = self.reverse_diffusion(
            prompt=speech_input_ids,
            target_len=target_length,
            phone_id=prompt_input_ids,
        ...
        if generated_codes.dim() == 2:
            generated_codes = generated_codes.unsqueeze(2)
